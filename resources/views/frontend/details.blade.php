...
@section('content')
    ...
    <style>
        ...
        */ .polling-responsive-table {
        ...
        /* Hide Trend pane without collapsing its width: */
        #pane-trend.hidden-pane {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 2) Build an array of { name, pct, color }, sort it descending by pct
            const candidates = template.map((t, i) => ({
                name: t.name,
                pct: avgs[i],
                color: t.color
            })).sort((a, b) => b.pct - a.pct);

            // 3) Extract legend labels, data and colors in sorted order
            const legendLabels = candidates.map(c => `${c.name} (${c.pct}%)`);
            const dataValues = candidates.map(c => c.pct);
            const colors = candidates.map(c => c.color);

            // 4) Render the doughnut
            ...
            new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                // first line: the full label (e.g. "John (75%)")
                                // label: ctx => ctx.label,
                                // second line: just the percent, and the marker shows the color
                                // afterLabel: ctx => `${ctx.parsed}%`
                            }
                        }
                    }
                },
